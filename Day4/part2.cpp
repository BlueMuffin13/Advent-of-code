...
int main(){
    ...
    for(int i=0; i<sections.size(); i+=2){
        ...
        //checking if beginning/end of range fits in other range
        if((e1start >= e2start && e1start <= e2fin) || (e2start >= e1start && e2start <= e1fin) || (e1fin >= e2start && e1fin <= e2fin) || (e2fin >= e1start && e2fin <= e1fin)){
            sum++;
        }
